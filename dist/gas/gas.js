...
//  html = html.split('<head>').join('<head><base target="_top">')
  html = html.split('<script type="module" src="./js/pjfu.js"></script>').join('<script>' + js + '</script>')
  // html = html.split('<script type="module" src="./js/main.js">').join('<script>' + mainJs() + '</script>')
  return html;
}
